...
//Rare exceptions to indicate a bug in the _alloyandroidiphonenavigationdemo code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_alloyandroidiphonenavigationdemo = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_alloyandroidiphonenavigationdemo]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.alloyandroidiphonenavigationdemo.module"
