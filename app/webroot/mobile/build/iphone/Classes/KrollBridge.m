...
NSString * AlloyAndroidiPhoneNavigationDemo$ModuleRequireFormat = @"(function(exports){"
...
@implementation AlloyAndroidiPhoneNavigationDemoObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_alloyandroidiphonenavigationdemo);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_alloyandroidiphonenavigationdemo gc];
}
...
{
	// create AlloyAndroidiPhoneNavigationDemo global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "AlloyAndroidiPhoneNavigationDemo" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_alloyandroidiphonenavigationdemo = [[AlloyAndroidiPhoneNavigationDemoObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_alloyandroidiphonenavigationdemo];
	
	NSString *_alloyandroidiphonenavigationdemoNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _alloyandroidiphonenavigationdemoNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_alloyandroidiphonenavigationdemo valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:AlloyAndroidiPhoneNavigationDemo$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.alloyandroidiphonenavigationdemo.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.alloyandroidiphonenavigationdemo.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
